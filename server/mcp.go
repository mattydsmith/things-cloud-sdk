...
func newMCPHandler() http.Handler {
	...
	s.AddTool(mcp.NewTool("things_create_task",
		...
		mcp.WithString("when",
			mcp.Description("Schedule: today, anytime, someday, or inbox (default)"),
	...
	s.AddTool(mcp.NewTool("things_edit_task",
		...
		mcp.WithString("when",
			mcp.Description("New schedule: today, anytime, someday, or inbox"),
			mcp.Enum("today", "anytime", "someday", "inbox"),
		),
		mcp.WithString("deadline",
			mcp.Description("New deadline in YYYY-MM-DD format"),
		),
		mcp.WithString("project",
