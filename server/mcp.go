...
func newMCPHandler() http.Handler {
	...
	s.AddTool(mcp.NewTool("things_create_task",
		...
		mcp.WithString("when",
			mcp.Description("When to start the task. Use this for most date-related requests — it controls which Things view the task appears in. 'today' = Today view, 'anytime' = Anytime view (triaged but no specific date), 'someday' = Someday view (deferred), 'inbox' = Inbox (default, needs triage)."),
			mcp.Enum("today", "anytime", "someday", "inbox"),
		),
	...
	s.AddTool(mcp.NewTool("things_edit_task",
		...
		mcp.WithString("when",
			mcp.Description("When to start the task. Use this for most date-related requests — it controls which Things view the task appears in. 'today' = Today view, 'anytime' = Anytime (triaged, no date), 'someday' = Someday (deferred), 'inbox' = Inbox (clears schedule)."),
			mcp.Enum("today", "anytime", "someday", "inbox"),
		),
		mcp.WithString("deadline",
