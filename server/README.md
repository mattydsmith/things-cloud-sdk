# Things Cloud API Server

A Go HTTP server that exposes [Things 3](https://culturedcode.com/things/) data via a REST API and an [MCP](https://modelcontextprotocol.io/) (Model Context Protocol) endpoint. Connects Claude and other AI assistants directly to your Things task manager.
...
The server acts as a bridge between Things Cloud and external clients. It syncs task data into a local SQLite database and exposes it through two interfaces:
...
| `mcp.go` | MCP server with 26 tool definitions and handlers |
| `write.go` | Write operations (create, edit, complete, trash, move) shared between REST and MCP |

### Sync model

The server maintains two sync channels with Things Cloud:

1. **Read sync** (`sync.Syncer`) — SQLite-backed incremental sync that pulls task state from Things Cloud. Called before every read operation to ensure fresh data.
...
This means you can freely make changes in the Things app and immediately use the API/MCP tools — no server restart required.

## Infrastructure

The server runs on [Fly.io](https://fly.io) with the following setup:

| Component | Detail |
|-----------|--------|
| **Region** | `lhr` (London) |
| **VM** | Shared CPU, 1 GB RAM |
| **Storage** | Persistent volume mounted at `/data` (SQLite WAL mode) |
...
| `API_KEY` | No | Bearer token for `/api/*` endpoints. If unset, no auth required. |
...
|----------|-------------|
| `GET /` | Health check — returns `{"service":"things-cloud-api","status":"ok"}` |
...
| `POST /api/tasks/complete` | `{"uuid":"..."}` | Mark task as completed |
| `POST /api/tasks/trash` | `{"uuid":"..."}` | Move task to trash |
...
All test scripts live in `tests/` and log results to `tests/test-results.log`.
...
See `tests/TEST_PLAN.md` for the full test plan with check-by-check detail.
...
mkdir -p /data  # or modify the DB path in main.go
./things-server
